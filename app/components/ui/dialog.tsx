...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-[9999] flex items-start justify-center overflow-y-auto bg-foreground/20 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className,
    )}
    {...props}
  />
...
>(({ className, withCloseButton = false, children, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay>
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          "m-4 grid w-full max-w-xl gap-6 rounded-lg border bg-white p-6 shadow-2xl lg:m-8 lg:mt-24",
          className,
        )}
        {...props}
      >
